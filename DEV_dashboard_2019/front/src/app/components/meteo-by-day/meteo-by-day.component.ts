...
@Component({
  selector: 'app-meteo-by-day',
  templateUrl: './meteo-by-day.component.html',
  styleUrls: ['./meteo-by-day.component.scss']
})
export class MeteoByDayComponent implements OnInit {
  @Input() meteoParams: any;
  @Input() itemId: any;
  ...
  ngOnInit(): void {
    ...
    if (this.meteoParams.ville != undefined) {
      this.ville = this.meteoParams.ville
    }
  ...
  changeMeteoParams(): void {
    ...
    this._httpClient.patch<any>('http://localhost:8080/widgets/' + this.itemId, body, httpOptions).subscribe(response => {
